...
    // Note: For a Team card, the client may be storing the team card's PRIMARY KEY here temporarily too.
    const storedTeamCardId = localStorage.getItem("team_card_id"); 
    
    if (stored && stored !== 'null' && stored !== 'undefined') {
      return Number(stored);
    } else if (storedTeamCardId && storedTeamCardId !== 'null' && storedTeamCardId !== 'undefined') {
      return Number(storedTeamCardId);
    }
    return null;
  };
...
    if (effectiveCardId && !effectiveTeamId) return "Myself"; // Only infer Myself if no Team ID is present
    return null;
  };
...
    : effectiveCardType === "Team" && effectiveCardId // Using effectiveCardId which holds the primary key
    ? `logo_preview:Team:${effectiveCardId}`
    : null;
...
    // NOTE: Team card logo fetching is currently omitted here. You may need a dedicated fetch endpoint for team logos later.
...
      console.error('Failed to crop image. Please try again.'); 
    }
  }, [imageSrc, croppedAreaPixels, setCroppedLogo, cacheKey]); // Added cacheKey dependency
...
      if (!token) {
        console.error("Authentication required. Please log in again."); // Replaced alert
        return false;
      }
...
        if (!type) {
          console.error("Missing card type. Please re-open Step 1."); // Replaced alert
          return false;
        }
        formData.append("cardType", type);
      if (type === "Myself") {
        if (!effectiveCardId) {
          console.error("Missing card ID. Please complete Step 1 first."); // Replaced alert
          return false;
        }
        formData.append("cardId", String(effectiveCardId));
      } else if (type === "Team") {
        if (!effectiveCardId) { // Use effectiveCardId which holds the team_cards.id primary key
          console.error("Missing team card primary ID."); // Added error message
          return false;
        }
        // The server's uploadLogo.js expects 'cardId' to update the 'id' column for a Team card
        formData.append("cardId", String(effectiveCardId)); // <-- CRITICAL FIX: Add cardId
        // Optionally still send teamId, but it is not used in the server's UPDATE query
        if (effectiveTeamId) {
          formData.append("teamId", String(effectiveTeamId));
        } else {
          console.warn("effectiveTeamId is missing, but proceeding with effectiveCardId.");
        }
      }
...
        // Replaced alert
        console.error(result.error || "Failed to upload logo");
        return false;
      }
    } catch (e) {
      console.error("Upload failed with error:", e);
      // Replaced alert
      console.error("Network error occurred while uploading logo");
      return false;
    }
  };
...
      // IMPORTANT: Replaced window.confirm() with custom error handling as per instructions
      console.warn("Upload failed. Continuing to next step with local image preview.");
      navigate("/create/primary-color", {
        state: {
          cardType,
          cardId: effectiveCardId,
